...
<Note> 
   When using `"reserved"` as the account type, set the `amount` field to `0`. This indicates that the account can accept     variable amounts rather than being tied to a fixed amount per transaction. 
</Note>
